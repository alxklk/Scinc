...
int main()
{
	...
	gui.AddButton("Go", 20,20,30,20,'go__').SetActionCB([](SWidget*p)->int
	{
		SWidget& self=*p;
		if(*(self.text)=='G')
		{
	...
	gui.AddSelect("Select A", 20,50,80,20,'sel0',1,&sel).SetActionCB([](SWidget*p)->int{p->text="002";return 0;});
	gui.AddSelect("Select B", 20,80,80,20,'sel1',2,&sel);
	gui.AddCheck("Check 0", 20,110,80,20,'ch_0',&ch0).SetActionCB([](SWidget*p)->int{p->text="001";return 0;});
	...
	gui.AddSlide("", 130,30,15,100,'val1',&val[1],true,0,1).SetActionCB([](SWidget*p)->int{printf("%f\n",*p->fDest);return 0;});
	...
	gui.AddLineEdit(editLine, 20,190,100,20).SetActionCB([](SWidget*p)->int{printf("'%s'\n",p->text);return 0;});
